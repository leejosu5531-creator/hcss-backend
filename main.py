...
def save_layout(req: LayoutModel):
    try:
        ...
        ensure_layout_sheet(sheets_service, file_id)

        now = dt.datetime.now(dt.timezone.utc).isoformat()
        values = sheets_service.spreadsheets().values().get(
            spreadsheetId=file_id, range=f"{LAYOUT_SHEET_NAME}!A:D"
        ).execute().get("values", [])

        existing_row = None
        for idx, row in enumerate(values[1:], start=2):
            if len(row) >= 2 and row[0] == req.date_str and str(row[1]) == str(req.total_seats):
                existing_row = idx
                break

        payload = json.dumps(req.rows, ensure_ascii=False, separators=(",", ":"))
        body = {"values":[[req.date_str, str(req.total_seats), now, payload]]}

        if existing_row:
            sheets_service.spreadsheets().values().update(
                spreadsheetId=file_id,
                range=f"{LAYOUT_SHEET_NAME}!A{existing_row}:D{existing_row}",
        ...
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "saved_at":now,"code_version":"2026-09-18-HCSS-final-layout-save-v2"}
...
def load_layout(req: LayoutModel):
    try:
        ...
        for row in reversed(values[1:]):
            if len(row) >= 4 and row[0] == req.date_str and str(row[1]) == str(req.total_seats):
                return {"status":"success","date":req.date_str,
                        "total_seats":req.total_seats,"rows":json.loads(row[3]),
                        "saved_at":row[2],"code_version":"2026-09-18-HCSS-final-layout-save-v2"}

        return {"status":"not_found","date":req.date_str,"total_seats":req.total_seats}
    except Exception as e:
        logger.exception("수동배치 불러오기 실패")
        raise HTTPException(status_code=500, detail=f"Google 불러오기 실패: {e}")
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-18-HCSS-final-layout-save-v2"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-18-HCSS-final-layout-save-v2",
            "date": str(target),
