...
def run_allocation(req: RequestModel):
    try:
        ...
        # 실제 출석 인원(64명 등)에 딱 맞는 권장 정원: 아무도 성도석으로 빼지 않아도
        # 되는 최소 정원(=출석 인원 + 오르간 1)이다. 이보다 정원을 작게 잡으면
        # 초과 인원을 알토만 우선적으로 빼게 되어 있어, 그 과정에서 성비가 틀어져
        # 1,2열보다 3,4열이 오히려 커지는 역전 현상이 생길 수 있다.
        attending_total = sum(attending_by_part.values())
        recommended_total_seats = min(attending_total + 1, sum([18, 18, 17, 16]))
        ...
        return {
            ...
            "sungdoseok": leftover,
            "recommended_total_seats": recommended_total_seats
        }
