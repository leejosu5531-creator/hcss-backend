...
# A1:G500 처럼 행 수를 못박아두면, 저장이 쌓여 500행을 넘는 순간부터 최신 저장분이
# 조회되지 않는 문제가 있었다(한 번 겪었던 버그). 그래서 열 전체(A:H)를 읽고 쓴다.
# H열(attending_total)은 "그 배치를 저장할 당시의 총 출석 인원" — 나중에 불러올 때
# 스프레드시트를 다시 읽어서 이 값과 비교해, 그 사이 출석부가 바뀌었는지 알려주는 데 쓴다.
LAYOUT_RANGE = f"'{LAYOUT_SHEET_NAME}'!A:H"
LAYOUT_HEADER = ["date","total_seats","saved_at","row","slot","part","name","attending_total"]
...
def ensure_layout_sheet(sheets_service, spreadsheet_id):
    ...
    if LAYOUT_SHEET_NAME not in names:
        ...
        sheets_service.spreadsheets().values().update(
            spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:H1",
            valueInputOption="RAW",
            body={"values": [LAYOUT_HEADER]}
        ).execute()


def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows, attending_total=None):
    """저장된 배치를 시트에 쓴다.

    예전에는 "전체 지우기(clear) → 새로 쓰기(update)" 순서였고, 이 함수 전체를
    with_retries로 감싸서 실패하면 처음부터 다시 실행했다. 그런데 clear는 성공했는데
    (콜드스타트 등으로) update가 실패하면, 시트는 이미 텅 빈 상태로 남는다. 이 상태에서
    "처음부터 다시" 재시도하면, 이번엔 "현재 저장된 내용(current)"을 다시 읽을 때 방금
    지워진 빈 시트를 읽어버려서, 이번에 저장하려던 날짜 말고 다른 모든 날짜의 저장 배치가
    통째로 사라지는 사고로 이어질 수 있었다. (실제로 이렇게 여러 날짜의 저장 배치가 한꺼번에
    없어지는 문제가 보고됐다.)

    이제는 순서를 바꿔서: 먼저 새 값을 그대로 덮어쓰고(clear 없이), 그 다음에 "이전보다
    짧아져서 뒤에 남는 찌꺼기 행"만 있으면 그 부분만 지운다. 덮어쓰기는 몇 번을 다시
    시도해도 매번 같은 값을 쓸 뿐이라 안전하고(멱등적), 뒤쪽 찌꺼기 정리가 실패해도
    이미 새 데이터는 안전하게 저장된 뒤이므로 데이터가 사라지지 않는다.
    """
    ...
    # 읽기만 하는 단계라 몇 번을 재시도해도 안전하다.
    current = with_retries(_read_current, attempts=3, what="기존 저장 목록 읽기")
    prev_row_count = len(current)
    ...
    if current:
        ...
        for r in current[1:]:
            if len(r) >= 1 and r[0] == date_str:
                continue
            kept.append(r)
    values = kept or [LAYOUT_HEADER]
    at_val = str(attending_total) if attending_total is not None else ""
    for r_idx, row in enumerate(rows, start=1):
        for slot_idx, item in enumerate(row, start=1):
            part, name = item[0], item[1]
            values.append([date_str, str(total_seats), now, str(r_idx), str(slot_idx), str(part), str(name), at_val])

    def _write():
        sheets_service.spreadsheets().values().update(
            spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:H{max(1,len(values))}",
            valueInputOption="RAW", body={"values": values}
        ).execute()
    with_retries(_write, attempts=3, what="시트 쓰기")

    # 이번에 쓴 내용이 이전 시트 크기보다 짧으면(예: 이번엔 인원이 줄어든 배치), 그만큼
    # 뒤에 옛 데이터가 남아있을 수 있으니 그 부분만 지운다. 이 정리가 실패해도(재시도 후에도)
    # 이미 위에서 새 데이터 쓰기는 끝난 뒤이므로 데이터 유실로 이어지지 않는다.
    if prev_row_count > len(values):
        def _trim():
            sheets_service.spreadsheets().values().clear(
                spreadsheetId=spreadsheet_id,
                range=f"'{LAYOUT_SHEET_NAME}'!A{len(values)+1}:H{prev_row_count}",
                body={}
            ).execute()
        try:
            with_retries(_trim, attempts=2, what="이전 찌꺼기 행 정리")
        except Exception:
            logger.exception("이전 저장분 찌꺼기 행 정리 실패 (새 데이터 저장 자체는 성공했으므로 무시하고 계속)")
...
def load_layout_from_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats):
    ...
    saved_attending_total = None   # 저장 당시 총 출석 인원(시트 H열) — 나중에 출석부가 바뀌었는지 비교용
    for r in values[1:]:
    ...
    return {
        ...
        "total_seats": saved_total_seats,
        "attending_total": saved_attending_total,
    }
...
def save_layout(req: LayoutModel):
    try:
        ...
        # 저장하는 시점의 총 출석 인원을 같이 기록해 둔다. 나중에 이 배치를 다시 불러올 때
        # 스프레드시트를 새로 읽어서 이 값과 비교하면, 그 사이 출석부가 바뀌었는지 알 수 있다.
        attending_total=None
        try:
            target=dt.date.fromisoformat(req.date_str)
            vals=with_retries(lambda: get_sheet_data(sheets_service,file_id,force_refresh=True), what="출석부 읽기")
            people=read_attendance(vals,target)
            attending_total=sum(len(people.get(p,[])) for p in ("S","A","T","B"))
        except Exception:
            logger.exception("저장 시 출석 인원 확인 실패 (저장 자체는 계속 진행)")

        # 주의: save_layout_to_google_sheet 자체가 내부적으로(읽기/쓰기/정리 단계별로)
        # 안전하게 재시도하므로, 여기서 함수 전체를 with_retries로 한 번 더 감싸지 않는다.
        # 예전에는 이 함수 전체를 다시 감쌌었는데, 그러면 "지우기는 됐는데 쓰기만 실패"한
        # 상황에서 처음부터 재시도할 때 이미 비어버린 시트를 "현재 내용"으로 다시 읽어버려서
        # 다른 날짜들의 저장 배치가 한꺼번에 사라지는 사고로 이어질 수 있었다.
        saved_at=save_layout_to_google_sheet(
            sheets_service,file_id,req.date_str,req.total_seats,req.rows,attending_total
        )
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "attending_total":attending_total,
                "saved_at":saved_at,"code_version":"2026-09-26-HCSS-v16"}
...
def load_layout(req: LayoutModel):
    try:
        ...
        # 저장 당시 출석 인원(H열)과 방금 새로 읽은 현재 출석 인원을 비교한다.
        # 값이 다르면 "그 사이 출석부가 바뀐 것 같다"는 신호이므로 안내 문구를 같이 보낸다.
        attendance_warning=None
        saved_attending_total=saved.get("attending_total")
        if saved_attending_total not in (None,"") and people:
            try:
                saved_att_int=int(saved_attending_total)
                if saved_att_int != attending_count:
                    diff=attending_count - saved_att_int
                    attendance_warning=(
                        f"⚠ 저장 당시 출석 인원({saved_att_int}명)과 지금 스프레드시트의 출석 인원"
                        f"({attending_count}명)이 다릅니다({'+' if diff>0 else ''}{diff}명). "
                        f"출석부가 그 사이 바뀐 것 같으니 배치를 확인해 주세요."
                    )
        ...
        return {"status":"success","date":req.date_str,"total_seats":saved_seats,
                ...
                "code_version":"2026-09-26-HCSS-v16"}
...
def health_check():
    ...
    GET과 HEAD를 모두 명시적으로 받는다. UptimeRobot 무료 플랜의 HTTP(s) 모니터는
    기본적으로 HEAD 요청을 보내는데(GET 선택은 유료 플랜 전용), @app.get()만 쓰면
    이론적으로는 Starlette가 HEAD도 자동 처리해줘야 하지만 실제로 405가 나는 경우가
    있어 methods를 직접 지정해 확실하게 만들었다."""
    return {"status": "ok", "code_version": "2026-09-26-HCSS-v16"}
...
def run_allocation(req: RequestModel):
    try:
        target = dt.date.fromisoformat(req.date_str)
        # 좌석 자동배치는 항상 구글 스프레드시트를 새로 읽어온다. 예전에는 "출석부
        # 새로고침" 체크박스로 사용자가 직접 켜야 했는데, 자동배치를 실행하면서
        # 캐시된(최대 몇 분 전) 출석 데이터를 쓰는 건 혼란만 준다고 판단해
        # 체크박스 값과 상관없이 항상 새로 읽도록 했다.
        ...
        sheet_values = with_retries(
            lambda: get_sheet_data(sheets_service, file_id, force_refresh=True),
            what="출석부 읽기"
        )
        ...
        return {
            ...
            "refresh_mode": "force",   # 좌석 자동배치는 항상 새로 읽으므로 고정값
        }
    except Exception as e:
