...
def save_layout(req: LayoutModel):
    try:
        ...
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "saved_at":saved_at,"code_version":"2026-09-20-HCSS-layout-load-v5"}
...
def load_layout(req: LayoutModel):
    try:
        ...
        saved = load_layout_from_google_sheet(
            sheets_service, file_id, req.date_str, req.total_seats
        )
        if not saved:
            return {"status":"not_found","date":req.date_str,
                    "total_seats":req.total_seats,
                    "code_version":"2026-09-20-HCSS-layout-load-v5"}

        row_part_counts=[]
        for row in saved["rows"]:
            c={"S":0,"A":0,"T":0,"B":0,"ORG":0}
            for item in row:
                if item and len(item)>=1 and item[0] in c:
                    c[item[0]] += 1
            row_part_counts.append(c)

        attending_by_part={p:sum(r[p] for r in row_part_counts)
                           for p in ("S","A","T","B")}
        attending_count=sum(attending_by_part.values())
        sungdoseok={"S":[],"A":[],"T":[],"B":[]}
        summary_source="저장된 수동배치 기준"

        # Attendance summary is supplementary. Failure here must NOT
        # invalidate the saved manual layout.
        try:
            ...
            attending_count=sum(attending_by_part.values())
            summary_source="Google Sheets API"
        except Exception as summary_error:
            logger.warning("출석 요약 조회 실패(저장좌석은 정상 반환): %s", summary_error)
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-20-HCSS-layout-load-v5"}
