...
def allocate(people_dict, total_seats):
    ...
    female_overflow_names = list(female_overflow_names)  # 아래에서 수정할 수 있으므로 복사

    front_alto = [n for n in (front1 + front2) if part_of[n] == "A"]
    front_soprano = [n for n in (front1 + front2) if part_of[n] == "S"]

    # 알토/소프라노 인원이 홀수로 남으면, 1열 또는 2열에 억지로 1명 차이를 남기는 대신
    # 초과분(3열로 갈 인원) 풀에서 반대 파트 1명을 맞바꿔 와서 둘 다 짝수로 맞춘다.
    # 이렇게 하면 1열=2열이 총원뿐 아니라 파트별 인원까지 정확히 일치해서, 같은 파트가
    # 앞뒤로 정확히 겹쳐 설 수 있다. (맞바꿀 인원이 초과분에 없으면 어쩔 수 없이
    # 최대 1명 차이를 허용한다)
    if len(front_alto) % 2 == 1:
        swap_idx = next((i for i, n in enumerate(female_overflow_names) if part_of[n] == "S"), None)
        if swap_idx is not None:
            pulled_soprano = female_overflow_names.pop(swap_idx)
            pushed_alto = front_alto.pop()  # 우선순위가 가장 낮은 알토를 3열로
            female_overflow_names.append(pushed_alto)
            front_soprano.append(pulled_soprano)

    # 앞줄 존 안에서 알토/소프라노를 각각 절반씩 1열-2열에 나눈다(파트 구성까지 맞춤).
    # (위에서 짝수로 맞췄다면 1열=2열이 파트별로도 정확히 일치하고, 맞바꿀 인원이
    #  없어 홀수가 남았다면 알토는 1열이, 소프라노는 2열이 그 잉여를 가져가
    #  서로 상쇄시켜 최소한 총원만큼은 정확히 유지한다)
    alto_row1, alto_row2 = front_alto[0::2], front_alto[1::2]
    soprano_row2, soprano_row1 = front_soprano[0::2], front_soprano[1::2]
