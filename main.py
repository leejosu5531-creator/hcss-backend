...
def get_cached_attendance(sheet_values, date_str, target):
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-16-seat-direction-fix16-layout-save"}


@app.get("/api/diagnostic/allocate")
def diagnostic_allocate(
    date_str: str = Query(default="2026-09-06", description="검사할 출석 날짜 YYYY-MM-DD"),
    total_seats: int = Query(default=67, ge=1, description="검사할 찬양대석 수"),
    force_refresh: bool = Query(default=True, description="Google Sheets를 캐시 없이 직접 다시 읽을지 여부"),
):
    """자동배치의 각 단계를 읽기 전용으로 진단한다.

    /api/allocate 자체의 동작은 변경하지 않는다. 이 엔드포인트는
    인증 -> Drive 파일 찾기 -> Sheets 읽기 -> 출석 판정 -> 좌석배치
    순서에서 어느 단계가 실패하는지 확인하기 위한 임시 진단용이다.
    """
    stages = []
    started = time.time()
    target = None
    try:
        ...
        stages.append({"stage": "1_date_parse", "status": "OK", "elapsed_ms": round((time.time()-t0)*1000, 1)})

        # 2. Sheets service
        t0 = time.time()
        sheets_service = get_sheets_service()
        stages.append({"stage": "2_google_sheets_service", "status": "OK", "elapsed_ms": round((time.time()-t0)*1000, 1)})

        # 3. Drive service
        t0 = time.time()
        drive_service = get_drive_service()
        stages.append({"stage": "3_google_drive_service", "status": "OK", "elapsed_ms": round((time.time()-t0)*1000, 1)})

        # 4. 출석부 파일 ID
        t0 = time.time()
        file_id = get_cached_file_id(drive_service)
        stages.append({
            "stage": "4_attendance_file_lookup",
            "status": "OK",
            "elapsed_ms": round((time.time()-t0)*1000, 1),
            "file_id_suffix": str(file_id)[-8:] if file_id else None,
        })
        ...
        sheet_values = get_sheet_data(
            sheets_service, file_id, force_refresh=force_refresh
        )
        stages.append({
            "stage": "5_google_sheets_read",
            "status": "OK",
            "elapsed_ms": round((time.time()-t0)*1000, 1),
            "sheet_count": len(sheet_values) if isinstance(sheet_values, list) else None,
        })

        # 6. 날짜별 출석 판정
        t0 = time.time()
        people = get_cached_attendance(sheet_values, date_str, target)
        attending_by_part = {p: len(people[p]) for p in ("S", "A", "T", "B")}
        stages.append({
            "stage": "6_attendance_parse",
            "status": "OK",
            "elapsed_ms": round((time.time()-t0)*1000, 1),
            "date": date_str,
        ...
        # 문제 사례로 사용하던 이의헌의 현재 판정도 함께 표시
        target_person = "이의헌"
        person_part = None
        person_present = False
        for part in ("S", "A", "T", "B"):
            if target_person in people.get(part, []):
                person_part = part
                person_present = True
                break
        person_result = {
        ...
        stages.append({
            "stage": "7_seat_allocation",
            "status": "OK",
            "elapsed_ms": round((time.time()-t0)*1000, 1),
            "row_targets": rt,
            "organ_row": org + 1,
            "row_part_counts": row_part_counts,
            "sungdoseok_count": len(leftover),
        })

        return {
            "status": "success",
            "code_version": "2026-09-18-allocate-diagnostic-v4",
            "date": date_str,
            "total_seats": total_seats,
            "force_refresh": force_refresh,
            "data_source": "Google Sheets API",
            "stages": stages,
            "problem_case": person_result,
            "elapsed_ms_total": round((time.time()-started)*1000, 1),
        }
    except Exception as e:
        stage_name = "unknown"
        if stages:
            # 마지막으로 성공한 단계 다음이 실패 지점
            stage_name = stages[-1]["stage"] + " -> NEXT_FAILED"
        logger.exception("자동배치 진단 오류")
        return {
            "status": "error",
            "code_version": "2026-09-18-allocate-diagnostic-v4",
            "date": date_str,
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-16-seat-direction-fix16-layout-save",
            "date": str(target),
