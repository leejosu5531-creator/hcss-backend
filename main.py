...
def google_drive_service():
    # Render에서는 SERVICE_ACCOUNT_JSON 환경변수를 우선 사용합니다.
    # 로컬에서는 기존 service_account.json을 사용할 수 있습니다.
    ...
    try:
        if raw:
            ...
            if missing:
                raise ValueError("SERVICE_ACCOUNT_JSON에 필수 항목이 없습니다: " + ", ".join(missing))
        ...
        if "Invalid JWT Signature" in msg or "invalid_grant" in msg:
            raise RuntimeError(
                "Google 서비스 계정 인증키가 유효하지 않습니다. "
                "Render의 SERVICE_ACCOUNT_JSON에 새 서비스 계정 JSON 전체 내용을 등록했는지 확인하세요."
            )
        raise
