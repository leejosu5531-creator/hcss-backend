...
def load_layout_diagnostic(date: str):
    """브라우저에서 저장배치 존재 여부를 직접 확인."""
    try:
        sheets_service = get_sheets_service()
        drive_service = get_drive_service()
        file_id = get_cached_file_id(drive_service)
        saved = load_layout_from_google_sheet(sheets_service, file_id, date, None)
        if not saved:
            return {"status":"not_found","date":date,
                    "message":"해당 날짜의 저장된 배치가 없습니다.",
                    "code_version":"2026-09-22-HCSS-v14"}
        return {"status":"success","date":date,"saved":True,
                "total_seats":int(saved.get("total_seats") or 0),
                "saved_at":saved.get("saved_at"),
                "row_count":len(saved.get("rows") or []),
                "rows":saved.get("rows") or [],
                "code_version":"2026-09-22-HCSS-v14"}
    except Exception as e:
        logger.exception("저장배치 GET 진단 실패")
        raise HTTPException(status_code=500, detail=f"저장배치 GET 진단 실패: {e}")
...
def load_layout(req: LayoutModel):
    try:
        ...
        saved = load_layout_from_google_sheet(
            sheets_service, file_id, req.date_str, None
        )
        if not saved:
        ...
        if not 60 <= saved_seats <= 70:
            raise HTTPException(
                status_code=400,
                detail="저장된 찬양대석 정원이 60~70명 범위를 벗어났습니다."
            )
        ...
        for row in saved["rows"]:
            ...
            for item in row:
                if item and item[0] in c:
                    c[item[0]] += 1
            row_part_counts.append(c)

        # 저장된 좌석은 그대로 복원한다.
        # 성도석 명단은 저장된 layout에 포함되어 있으면 그대로 사용한다.
        sungdoseok = saved.get(
            "sungdoseok", {"S": [], "A": [], "T": [], "B": []
        })
