...
def find_drive_file(service):
    # 같은 이름의 파일이 여러 개 있으면 가장 최근 수정된 파일을 선택합니다.
    q = f"name = '{DRIVE_FILE_NAME.replace(chr(39), chr(92)+chr(39))}' and trashed = false"
    ...
    files = r.get("files", [])
    if not files:
        raise FileNotFoundError(f"Google Drive에서 '{DRIVE_FILE_NAME}'을 찾지 못했습니다.")
    return files[0]

def download_excel(service, file_id, destination):
    # 매 실행마다 Google Drive의 최신 파일을 실제로 다시 다운로드합니다.
    temp_destination = destination + ".new"
    request = service.files().get_media(fileId=file_id)
    with open(temp_destination, "wb") as fh:
