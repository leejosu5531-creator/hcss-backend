...
def parse_cell_date(val, target_year):
    ...
    # 2. 엑셀 날짜 일련번호(int / float)인 경우 (예: 45534)
    ...
    # YYYY-MM-DD, YYYY.MM.DD, YYYY/MM/DD 등 전체 날짜 포맷
    ...
    # MM-DD, MM/DD, M/D 등 월/일만 있는 포맷 (현재/대상 연도 조합)
    for fmt in ("%m-%d", "%m.%d", "%m/%d", "%m월 %d일", "%m월%d일"):
        try:
            parsed = dt.datetime.strptime(val_str, fmt).date()
            return parsed.replace(year=target_year)
        except ValueError:
            pass
...
def read_attendance(file_stream, target_date):
    ...
    if not date_col:
        raise ValueError(f"해당 날짜({target_date})를 출석부 3행에서 찾을 수 없습니다. 3행의 날짜 형식을 확인해주세요.")
