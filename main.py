...
def allocate(people_dict, total_seats):
    ...
    # 좌우 반전 정렬 (베이스 -> 테너 -> 알토 -> 소프라노 -> 오르간)
    reversed_part_order = {"B": 0, "T": 1, "A": 2, "S": 3, "ORG": 4}
    ...
    for r_idx in range(4):
        ...
        sorted_row = sorted(curr_row, key=lambda x: reversed_part_order[x[0]])
        final_rows.append(sorted_row)

        # 각 열별 파트별 배치 인원 집계
        p_counts = {p: sum(1 for part, _ in sorted_row if part == p) for p in ("S", "A", "T", "B", "ORG")}
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "date": str(target),
            "attending_count": sum(attending_by_part.values()), # 총 출석인원
            "attending_by_part": attending_by_part,            # 파트별 출석 인원 (소프라노, 알토, 테너, 베이스)
            "row_targets": rt,
            "organ_row": org + 1,
            "rows": rows,
            "row_part_counts": row_part_counts,                # 각 열마다 파트별 배치 인원
            "leftover": leftover
        }
