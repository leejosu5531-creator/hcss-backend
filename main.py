...
def load_layout(req: LayoutModel):
    try:
        ...
        for row in saved["rows"]:
            ...
            counts.append(c)
        # 저장된 수동 좌석은 그대로 반환하되, 성도석 이동 현황은 현재 출석부 기준으로
        # 다시 계산하여 화면 요약에 실제 이동자 이름이 표시되도록 한다.
        _, current_leftover, _, _, _, _ = allocate(people, req.total_seats)
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "rows":saved["rows"],"saved_at":saved["saved_at"],
                "attending_count":sum(ap.values()),"attending_by_part":ap,
                "row_part_counts":counts,
                "sungdoseok":current_leftover,
