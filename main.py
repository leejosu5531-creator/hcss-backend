...
def allocate(people_dict, total_seats):
    ...
    # 오르간 배치 열 결정: 사람 배치가 다 끝난 뒤, 실제 3열/4열 인원수를 비교해서
    # 더 적은 쪽에 오르간을 배정한다. 오르간을 미리(사람 배치 전) 정해두면
    # 화면 표시 총원(인원+오르간)이 오히려 불균형해지는 문제가 있었다.
    real_row3, real_row4 = len(rows[2]), len(rows[3])
    if real_row3 < real_row4:
        organ_row = 2
    elif real_row4 < real_row3:
        organ_row = 3
    else:
        # 3열=4열로 동률인 경우: 오르간을 3열에 넣으면 화면 표시가 2열보다 커질 수 있으니,
        # 그 경우에는 4열에 넣어 2-3열 경계를 먼저 지킨다.
        organ_row = 3 if real_row3 == len(rows[1]) else 2
