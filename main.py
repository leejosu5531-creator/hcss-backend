...
def download_excel_with_diagnostics(service, file_id, attempts=3, wait_seconds=2):
    """강제 새로고침 시 Drive XLSX 본문을 여러 번 직접 내려받아 실제 본문 변화를 진단한다.
    modifiedTime과 무관하게 매번 get_media()를 호출한다.
    """
    results = []
    last_bytes = None
    for i in range(1, attempts + 1):
        ...
        dbg = debug_attendance_cell(data, dt.date.fromisoformat(_diagnostic_target_date), _diagnostic_target_name, _diagnostic_target_part) if _diagnostic_target_date else None
        results.append({"attempt": i, "read_at": read_at, "size": len(data), "sha256": sha, "attendance_debug": dbg})
        last_bytes = data
        if i < attempts:
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-12-force-body-refresh-v3"}
    ...
    try:
        ...
        try:
            ...
            if req.force_refresh:
                ...
                mtime = meta.get("modifiedTime")
                file_bytes, download_diagnostics = download_excel_with_diagnostics(service, file_id, attempts=3, wait_seconds=2)
        ...
        return {
            "status": "success",
            "code_version": "2026-09-12-force-body-refresh-v3",
            "date": str(target),
