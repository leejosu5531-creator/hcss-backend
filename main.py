...
# 열/파트별 기본 좌석 명단 + 시작 방향(사용자가 지정한 좌/우 시작점).
#  "left"  = 이 목록 순서가 그대로 "왼쪽부터" 채워지는 순서
#  "right" = 이 목록 순서가 "오른쪽부터" 채워지는 순서(배열로 쓸 때는 뒤집는다)
# 여기 등록된 사람은 해당 열/파트가 "기본 자리"이며, 출석하면 우선 그 자리로 간다.
ROW_PART_LISTS = {
    (1, "A"): ("left",  ["심윤정", "박혜숙", "서정영", "오정민", "박소윤", "유미영", "노인숙", "김혜진", "임은애"]),
    (1, "S"): ("right", ["남현숙", "김경미", "백시원", "박지현", "이서연", "이은진", "고태옥", "황진서", "장영자", "정은희", "황재연"]),
    (2, "A"): ("left",  ["우다연", "김지영", "방민주", "김준희", "강미화", "박미선", "김현경", "권순예", "조경화", "장형미"]),
    (2, "S"): ("right", ["정진순", "김은영", "조한주", "손국희", "김서영", "김은희", "박진희", "황주경", "김은현", "박도희"]),
    (3, "B"): ("left",  ["이재관", "정동근", "목현성", "전병대", "성준호", "권상대b", "이입교"]),
    (3, "S"): ("right", ["김지은", "노희령", "류은채", "박유림", "윤선미"]),
    (3, "T"): ("right", ["강종훈", "권재훈", "하효동", "권영훈", "김기형"]),
    (4, "T"): ("right", ["시진규", "남명호", "이병륜", "김동근", "문지민", "김경남", "박상훈", "권상대a"]),
}
 
# 4열 베이스는 특수 케이스: 좌우 양끝을 기준점으로 중앙을 향해 채운다.
# 앞 절반(김대성~최한열)은 왼쪽 끝에서 안쪽으로, 뒤 절반(장현재~이종성)은
# 오른쪽 끝에서 안쪽으로 채운다.
ROW4_BASS_LEFT = ["김대성", "김기훈", "이의헌", "최한열"]
ROW4_BASS_RIGHT = ["장현재", "박선일", "박동민", "이종성"]
...
def allocate(people_dict, total_seats):
    ...
    alto_row1_pool = [n for n in BASE_SEAT_ORDER[1] if n in all_alto]
    alto_row2_pool = [n for n in BASE_SEAT_ORDER[2] if n in all_alto]
    ...
    s_row1_pool = [n for n in BASE_SEAT_ORDER[1] if n in all_soprano]
    s_row2_pool = [n for n in BASE_SEAT_ORDER[2] if n in all_soprano]
