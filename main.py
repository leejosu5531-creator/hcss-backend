...
def allocate(people_dict, total_seats):
    ...
    if not 60 <= total_seats <= 70:
        raise ValueError("찬양대석 정원은 60~70명 범위로만 설정할 수 있습니다.")
...
class LayoutModel(BaseModel):
    date_str: str
    total_seats: int = Field(default=67, ge=60, le=70)
...
def load_layout_from_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats=None):
    """날짜를 기준으로 저장배치를 찾고, 같은 날짜의 최신 저장본을 반환한다."""
    try:
        ensure_layout_sheet(sheets_service, spreadsheet_id)
    ...
    candidates=[]
    for r in values[1:]:
        if len(r)<7 or r[0]!=date_str: continue
        try:
            candidates.append((r[2] or "", int(r[1]), int(r[3]), int(r[4]), [r[5],r[6]]))
        except Exception:
            continue
    if not candidates: return None
    latest=max(x[0] for x in candidates)
    chosen=[x for x in candidates if x[0]==latest]
    saved_seats=chosen[0][1]
    max_row=max(x[2] for x in chosen)
    rows=[[] for _ in range(max_row)]
    for _,_,ri,si,item in sorted(chosen,key=lambda x:(x[2],x[3])):
        rows[ri-1].append(item)
    return {"rows":rows,"saved_at":latest,"total_seats":saved_seats}
...
def save_layout(req: LayoutModel):
    try:
        ...
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "saved_at":saved_at,"code_version":"2026-09-19-HCSS-final-v5"}
...
def load_layout(req: LayoutModel):
    try:
        sheets_service=get_sheets_service(); drive_service=get_drive_service()
        file_id=get_cached_file_id(drive_service)
        saved=load_layout_from_google_sheet(sheets_service,file_id,req.date_str,None)
        if not saved: return {"status":"not_found","date":req.date_str,"requested_total_seats":req.total_seats}
        saved_total=int(saved["total_seats"])
        if not 60<=saved_total<=70: raise HTTPException(status_code=400,detail="저장된 찬양대석 정원이 60~70명 범위를 벗어났습니다.")
        target=dt.date.fromisoformat(req.date_str)
        sheet_values=get_sheet_data(sheets_service,file_id,force_refresh=True)
        people=read_attendance(sheet_values,target)
        ...
        for row in saved["rows"]:
            ...
            counts.append(c)
        _,leftover,_,_,_,_=allocate(people,saved_total)
        return {"status":"success","date":req.date_str,"total_seats":saved_total,"rows":saved["rows"],"saved_at":saved["saved_at"],"attending_count":sum(ap.values()),"attending_by_part":ap,"row_part_counts":counts,"sungdoseok":leftover,"data_source":"Google Sheets API","layout_source":"Google Sheets saved layout","refresh_mode":"force","code_version":"2026-09-19-HCSS-v8-auto-load-by-date"}
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-19-HCSS-final-v5"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-19-HCSS-final-v5",
            "date": str(target),
