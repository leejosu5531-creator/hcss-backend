...
# 배포 확인용: 브라우저에서 /health 를 열어 이 값이 바뀌었는지 보면 새 코드가 반영된 것이다.
CODE_VERSION = "2026-09-20-HCSS-v9"
...
_cache = {
    ...
    "sheet_values_at": 0.0,     # 위 값을 읽은 시각(time.time())
...
# googleapiclient가 쓰는 httplib2.Http 는 스레드 안전하지 않다. FastAPI 는 일반 def 엔드포인트를
# 스레드풀에서 동시에 실행하므로(페이지를 열자마자 자동 불러오기 + 버튼 클릭 등),
# 서비스 객체 하나를 모든 요청이 공유하면 SSL/연결 오류나 응답 끊김이 생길 수 있다.
# 그래서 스레드마다 자기 서비스 객체를 만들어 재사용한다.
...
def get_sheet_data(sheets_service, spreadsheet_id, force_refresh=False):
    ...
    with _cache_lock:
        ...
        _cache["sheet_values_at"] = time.time()
...
# 좌석 배치 로직
# ---------------------------------------------------------------------------
# 열별 최대 좌석(1~4열). 합계가 찬양대석 정원의 최대값이다. (1열=2열 >= 3열 >= 4열 규칙 유지)
# 정원 한도를 바꾸려면 여기와 index.html 의 MAX_SEATS 를 함께 바꾼다.
ROW_CAPS = (18, 18, 17, 17)   # 합계 70
...
# 예전에는 A1:G500 이었다. 한 배치가 약 68행이라 7~8개 배치가 쌓이면 500행을 넘어
# 최신 저장분이 조회되지 않았으므로 열 전체(A:G)를 읽고 쓴다.
LAYOUT_RANGE = f"'{LAYOUT_SHEET_NAME}'!A:G"


def ensure_layout_sheet(sheets_service, spreadsheet_id):
    """저장용 시트가 없으면 생성하고 헤더를 준비한다. (sheetId, 현재 행 수)를 돌려준다."""
    ...
    for x in meta.get("sheets", []):
        ...
        if props.get("title") == LAYOUT_SHEET_NAME:
            return props.get("sheetId"), props.get("gridProperties", {}).get("rowCount", 1000)
    body = {"requests": [{"addSheet": {"properties": {"title": LAYOUT_SHEET_NAME}}}]}
    resp = sheets_service.spreadsheets().batchUpdate(spreadsheetId=spreadsheet_id, body=body).execute()
    props = resp.get("replies", [{}])[0].get("addSheet", {}).get("properties", {})
...
def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows):
    sheet_id, row_count = ensure_layout_sheet(sheets_service, spreadsheet_id)
    now = dt.datetime.now(dt.timezone.utc).isoformat()
    # 기존 동일 날짜+정원의 저장분 제거 후 새 배치로 교체
    ...
    if current:
        ...
        for r in current[1:]:
            if len(r) >= 2 and r[0] == date_str and str(r[1]) == str(total_seats):
                continue
            kept.append(r)
    ...
    # 저장분이 쌓여 시트 기본 행 수(1000)를 넘으면 쓰기가 실패하므로 먼저 행을 늘린다.
    needed = len(values)
    if sheet_id is not None and needed > row_count:
        sheets_service.spreadsheets().batchUpdate(
            spreadsheetId=spreadsheet_id,
            body={"requests": [{"appendDimension": {
                "sheetId": sheet_id, "dimension": "ROWS", "length": needed - row_count + 200}}]}
        ).execute()
    ...
    sheets_service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:G{max(1,needed)}",
...
def load_layout_from_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats):
    """저장된 배치를 읽는다. 저장 기록이 없으면 None.
    (조회만 하므로 시트를 새로 만들지 않는다. 구글 API 오류는 삼키지 않고 그대로 올려서
    '저장된 배치가 없음'과 '읽기 실패'가 구분되게 한다.)"""
    try:
        values = sheets_service.spreadsheets().values().get(
            spreadsheetId=spreadsheet_id, range=LAYOUT_RANGE
        ).execute().get("values", [])
    except HttpError as ex:
        # 저장용 시트(HCSS_수동배치)가 아직 없으면 range 해석 오류(400)가 난다 → 저장한 적 없음
        if getattr(ex, "resp", None) is not None and ex.resp.status == 400 and "Unable to parse range" in str(ex):
            return None
        raise
    found = []
    saved_at = None
    for r in values[1:]:
        if len(r) >= 7 and r[0] == date_str and str(r[1]) == str(total_seats):
            try:
    ...
    return {"rows": rows, "saved_at": saved_at}
...
def save_layout(req: LayoutModel):
    try:
        ...
        verified=load_layout_from_google_sheet(sheets_service,file_id,req.date_str,req.total_seats)
        if not verified:
            raise RuntimeError(f"저장 직후 검증 실패: {req.date_str}/{req.total_seats}")
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "saved_at":saved_at,"verified":True,"code_version":CODE_VERSION}
    ...
    try:
        ...
        for attempt in range(3):
            try:
                ...
                last_err=None
                break            # 저장분이 없어도(None) 오류가 아니므로 더 기다리지 않는다
            except Exception as ex:
                last_err=ex
                logger.warning("저장배치 읽기 재시도 %s: %s",attempt+1,ex)
                if attempt<2:
                    time.sleep(1.5)
        ...
        for row in saved["rows"]:
            ...
            counts.append(c)
        ap={p:sum(r[p] for r in counts) for p in ("S","A","T","B")}
        total=sum(ap.values())
        source="저장된 수동배치 기준"
        leftover={"S":[],"A":[],"T":[],"B":[]}   # 정원 초과로 성도석에 앉는 인원
        try:
            target=dt.date.fromisoformat(req.date_str)
            # 출석부 전체(4개 시트)를 매번 다시 읽으면 느리다. 최근 5분 안에 읽은 값이 있으면 재사용한다.
            # (엑셀/시트를 방금 고쳤다면 '좌석 자동배치 실행'에서 출석부 새로고침을 체크하면 된다)
            with _cache_lock:
                fresh = (time.time() - _cache["sheet_values_at"]) < ATTENDANCE_CACHE_TTL
            vals=get_sheet_data(sheets_service,file_id,force_refresh=not fresh)
            people=read_attendance(vals,target)
            ap={p:len(people[p]) for p in ("S","A","T","B")}
            total=sum(ap.values())
            # 저장된 배치에는 성도석 명단이 없으므로, 오늘 출석자 중 저장된 좌석에 없는 사람을 성도석으로 본다.
            seated={item[1] for row in saved["rows"] for item in row if item and len(item)>1}
            leftover={p:[n for n in people[p] if n not in seated] for p in ("S","A","T","B")}
            source="Google Sheets API"
        except Exception as ex:
            logger.warning("출석요약 실패, 저장좌석은 유지: %s",ex)
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,
                "rows":saved["rows"],"saved_at":saved["saved_at"],
                "attending_count":total,"attending_by_part":ap,
                "row_part_counts":counts,"sungdoseok":leftover,
                "data_source":source,"code_version":CODE_VERSION}
    except Exception as e:
        logger.exception("수동배치 불러오기 실패")
        raise HTTPException(status_code=500,detail=f"Google 수동배치 불러오기 실패: {e}")
...
def run_allocation(req: RequestModel):
    try:
        ...
        recommended_max_seats = min(
            attending_total + 1, sum(ROW_CAPS)
        )
