...
class LayoutModel(BaseModel):
    ...
    attending_by_part: dict = Field(default_factory=dict)
    row_part_counts: list = Field(default_factory=list)
    row_targets: list = Field(default_factory=list)
    organ_row: Optional[int] = None
    sungdoseok: dict = Field(default_factory=dict)
    row_slots: list = Field(default_factory=list)

LAYOUT_SHEET_NAME = "HCSS_수동배치"
LAYOUT_RANGE = f"'{LAYOUT_SHEET_NAME}'!A1:O2000"
LAYOUT_HEADERS = ["date","total_seats","saved_at","row","slot","part","name","record_type","attending_count","attending_S","attending_A","attending_T","attending_B","row_part_counts_json","row_slots_json"]
...
def ensure_layout_sheet(sheets_service, spreadsheet_id):
    ...
    sheets_service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:O1",
        valueInputOption="RAW", body={"values": [LAYOUT_HEADERS]}
    ).execute()

def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows, attending_count=None, attending_by_part=None, row_part_counts=None, row_slots=None, row_targets=None, organ_row=None, sungdoseok=None):
    ensure_layout_sheet(sheets_service, spreadsheet_id)
    now = dt.datetime.now(dt.timezone.utc).isoformat()
    current = sheets_service.spreadsheets().values().get(spreadsheetId=spreadsheet_id, range=LAYOUT_RANGE).execute().get("values", [])
    kept=[LAYOUT_HEADERS]
    for r in current[1:]:
        if len(r)>=2 and r[0]==date_str and str(r[1])==str(total_seats): continue
        kept.append(r)
    p = attending_by_part or {}
    rpc_json=json.dumps(row_part_counts or [], ensure_ascii=False, separators=(",",":"))
    slots_json=json.dumps(row_slots or [], ensure_ascii=False, separators=(",",":"))
    values=kept
    values.append([date_str,str(total_seats),now,"","","","","META",str(attending_count if attending_count is not None else ""),str(p.get("S",0)),str(p.get("A",0)),str(p.get("T",0)),str(p.get("B",0)),rpc_json,slots_json])
    for r_idx,row in enumerate(rows,start=1):
        for slot_idx,item in enumerate(row,start=1):
            if not item or len(item)<2: continue
            values.append([date_str,str(total_seats),now,str(r_idx),str(slot_idx),str(item[0]),str(item[1]),"SEAT","","","","","","",""])
    sheets_service.spreadsheets().values().clear(spreadsheetId=spreadsheet_id, range=LAYOUT_RANGE, body={}).execute()
    sheets_service.spreadsheets().values().update(spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:O{max(1,len(values))}", valueInputOption="RAW", body={"values":values}).execute()
    return now

def load_layout_from_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats):
    try:
        ensure_layout_sheet(sheets_service, spreadsheet_id)
        values=sheets_service.spreadsheets().values().get(spreadsheetId=spreadsheet_id, range=LAYOUT_RANGE).execute().get("values",[])
    except Exception: return None
    found=[]; saved_at=None; meta=None
    for r in values[1:]:
        if len(r)<2 or r[0]!=date_str or str(r[1])!=str(total_seats): continue
        typ=r[7] if len(r)>7 else ""
        if typ=="META":
            saved_at=r[2] if len(r)>2 else None
            try:
                meta={"attending_count":int(r[8]) if len(r)>8 and str(r[8]).strip() else None,
                      "attending_by_part":{"S":int(r[9]) if len(r)>9 and str(r[9]).strip() else 0,"A":int(r[10]) if len(r)>10 and str(r[10]).strip() else 0,"T":int(r[11]) if len(r)>11 and str(r[11]).strip() else 0,"B":int(r[12]) if len(r)>12 and str(r[12]).strip() else 0},
                      "row_part_counts":json.loads(r[13]) if len(r)>13 and r[13] else [],
                      "row_slots":json.loads(r[14]) if len(r)>14 and r[14] else []}
            except Exception as e:
                logger.warning(f"수동배치 메타데이터 파싱 실패: {e}"); meta=None
            continue
        if len(r)>=7:
            try: ri,si=int(r[3]),int(r[4])
            except Exception: continue
            if ri>=1 and si>=1 and r[5] and r[6]: found.append((ri,si,[r[5],r[6]]))
            if len(r)>2: saved_at=r[2]
    if not found: return None
    max_row=max(x[0] for x in found); rows=[[] for _ in range(max_row)]
    for ri,si,item in sorted(found,key=lambda x:(x[0],x[1])):
        rows[ri-1].append(item)
    result={"rows":rows,"saved_at":saved_at}
    if meta: result.update(meta)
    return result

@app.post("/api/layout/save")
def save_layout(req: LayoutModel):
    try:
        dt.date.fromisoformat(req.date_str)
        drive_service=get_drive_service(); file_id=get_cached_file_id(drive_service); sheets_service=get_sheets_service()
        saved_at=save_layout_to_google_sheet(sheets_service,file_id,req.date_str,req.total_seats,req.rows,req.attending_count,req.attending_by_part,req.row_part_counts,req.row_slots,req.row_targets,req.organ_row,req.sungdoseok)
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,"saved_at":saved_at}
    except Exception as e:
        logger.exception(f"수동배치 저장 오류: {e}"); raise HTTPException(status_code=400,detail=str(e))
...
def load_layout(req: LayoutModel):
    try:
        ...
        if not saved: return {"status":"not_found","date":req.date_str,"total_seats":req.total_seats}
        return {"status":"success","date":req.date_str,"total_seats":req.total_seats,"rows":saved["rows"],"saved_at":saved.get("saved_at"),"attending_count":saved.get("attending_count"),"attending_by_part":saved.get("attending_by_part",{}),"row_part_counts":saved.get("row_part_counts",[]),"row_slots":saved.get("row_slots",[])}
    except Exception as e:
        logger.exception(f"수동배치 불러오기 오류: {e}"); raise HTTPException(status_code=400,detail=str(e))
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-16-layout-persistence-v2"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-16-layout-persistence-v2",
            "date": str(target),
