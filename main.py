...
def get_excel_bytes(service, file_id, expect_change_from=None, max_retries=3, retry_delay=2.0):
    ...
    잠깐(retry_delay초) 기다렸다가 다시 확인한다. 구글 시트는 화면에는 "저장 중"이
    바로 뜨지만 드라이브 API로 읽을 수 있는 시점까지 1~수 초의 지연이 있을 수 있기
    때문이다."""
    mtime = None
    ...
    with _cache_lock:
        ...
        cached_bytes_exists = _cache["file_bytes"] is not None
        if cached_bytes_exists and cached_mtime == mtime:
            logger.info(f"[캐시] 파일 변경 없음 감지 -> 캐시된 바이트 재사용 "
                        f"(file_id={file_id}, mtime={mtime})")
            return _cache["file_bytes"], mtime
    logger.info(f"[다운로드] 새로 다운로드 진행 "
    ...
    logger.info(f"[다운로드 완료] {len(file_bytes)} bytes, mtime={mtime}로 캐시 갱신")
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-12-fix9-async-mtime-polling"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        try:
            file_id = get_cached_file_id(service)
            file_bytes, mtime = get_excel_bytes(service, file_id, expect_change_from=expect_change_from)
        except Exception:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-12-fix9-async-mtime-polling",
            "date": str(target),
