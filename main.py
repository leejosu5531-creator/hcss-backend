...
def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows,
    ...
    header = ["date","total_seats","saved_at","row","slot","part","name",
              "attending_count","attending_by_part","row_part_counts","sungdoseok"]
    ...
    sheets_service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:K{max(1,len(values))}",
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-17-seat-summary-save-seed-history-sungdoseok-v1"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-17-seat-summary-save-seed-history-sungdoseok-v1",
            "date": str(target),
