...
async def unhandled_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        ...
        content={
            ...
            "code_version": "2026-09-17-v12-date-settings-cache-v1",
        },
    )
...
DATE_SETTINGS_SHEET_NAME = "HCSS_날짜설정"
DATE_SETTINGS_RANGE = f"'{DATE_SETTINGS_SHEET_NAME}'!A1:C10000"

_layout_history_cache = {"data": None, "cached_at": 0.0}
_LAYOUT_HISTORY_CACHE_TTL = 60.0
...
def ensure_date_settings_sheet(sheets_service, spreadsheet_id):
    """날짜별 마지막 좌석 정원을 저장하는 시트를 준비한다."""
    meta = sheets_service.spreadsheets().get(spreadsheetId=spreadsheet_id, fields="sheets.properties").execute()
    names = [x.get("properties", {}).get("title") for x in meta.get("sheets", [])]
    if DATE_SETTINGS_SHEET_NAME not in names:
        body = {"requests": [{"addSheet": {"properties": {"title": DATE_SETTINGS_SHEET_NAME}}}]}
        sheets_service.spreadsheets().batchUpdate(spreadsheetId=spreadsheet_id, body=body).execute()
        sheets_service.spreadsheets().values().update(
            spreadsheetId=spreadsheet_id, range=f"'{DATE_SETTINGS_SHEET_NAME}'!A1:C1",
            valueInputOption="RAW",
            body={"values": [["date", "total_seats", "saved_at"]]}
        ).execute()

def save_date_setting(sheets_service, spreadsheet_id, date_str, total_seats):
    ensure_date_settings_sheet(sheets_service, spreadsheet_id)
    now = dt.datetime.now(dt.timezone.utc).isoformat()
    sheets_service.spreadsheets().values().append(
        spreadsheetId=spreadsheet_id, range=f"'{DATE_SETTINGS_SHEET_NAME}'!A:C",
        valueInputOption="RAW", insertDataOption="INSERT_ROWS",
        body={"values": [[date_str, str(total_seats), now]]}
    ).execute()
    return now

def load_date_setting(sheets_service, spreadsheet_id, date_str):
    try:
        ensure_date_settings_sheet(sheets_service, spreadsheet_id)
        values = sheets_service.spreadsheets().values().get(
            spreadsheetId=spreadsheet_id, range=DATE_SETTINGS_RANGE
        ).execute().get("values", [])
    except Exception:
        return None
    candidates = []
    for r in values[1:]:
        if len(r) < 3 or str(r[0]) != str(date_str) or not r[2]:
            continue
        try:
            seats = int(r[1]); ts = dt.datetime.fromisoformat(str(r[2]).replace("Z", "+00:00"))
            candidates.append((ts, seats))
        except Exception:
            continue
    if not candidates:
        return None
    ts, seats = max(candidates, key=lambda x:x[0])
    return {"date": date_str, "total_seats": seats, "saved_at": ts.isoformat()}
...
def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows,
    ...
    _layout_history_cache["data"] = None
    _layout_history_cache["cached_at"] = 0.0
    return now
...
def load_all_layout_history_from_google_sheet(sheets_service, spreadsheet_id):
    ...
    _layout_history_cache["data"] = records
    _layout_history_cache["cached_at"] = time.monotonic()
    return records
...
def save_layout(req: LayoutModel):
    try:
        ...
        setting_saved_at = save_date_setting(sheets_service, file_id, req.date_str, req.total_seats)
        return {"status":"success", "date":req.date_str, "total_seats":req.total_seats, "saved_at":saved_at, "setting_saved_at":setting_saved_at}
    except Exception as e:
...
@app.post("/api/date-setting/load")
def date_setting_load(req: LayoutModel):
    try:
        dt.date.fromisoformat(req.date_str)
        drive_service = get_drive_service()
        file_id = get_cached_file_id(drive_service)
        sheets_service = get_sheets_service()
        saved = load_date_setting(sheets_service, file_id, req.date_str)
        if not saved:
            return {"status":"not_found", "date":req.date_str}
        return {"status":"success", **saved}
    except Exception as e:
        logger.error(f"날짜별 좌석정원 조회 오류: {e}")
        raise HTTPException(status_code=400, detail=str(e))

@app.post("/api/date-setting/save")
def date_setting_save(req: LayoutModel):
    try:
        dt.date.fromisoformat(req.date_str)
        drive_service = get_drive_service()
        file_id = get_cached_file_id(drive_service)
        sheets_service = get_sheets_service()
        saved_at = save_date_setting(sheets_service, file_id, req.date_str, req.total_seats)
        return {"status":"success", "date":req.date_str, "total_seats":req.total_seats, "saved_at":saved_at}
    except Exception as e:
        logger.error(f"날짜별 좌석정원 저장 오류: {e}")
        raise HTTPException(status_code=400, detail=str(e))
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-17-v12-date-settings-cache-v1"}
