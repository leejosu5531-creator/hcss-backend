...
def allocate(people_dict, total_seats):
    ...
    # 1,2열 배치 후 남은 알토도 마찬가지로 정원을 넘더라도 3열에 전원 배치한다.
    # 알토는 4열에는 앉지 않으므로(테너/베이스만 4열 배치) 3열 정원 체크에 걸려 밀려나면
    # 성도석 외에 갈 곳이 없다. 전체 좌석에 여유가 있다면 성도석으로 보내지 않기 위해
    # 테너/베이스보다 먼저, 정원 체크 없이 확정 배치한다.
    for name in people["A"]:
        rows[2].append(("A", name))
    people["A"] = []

    # 3열/4열의 지정석(BASE_SEAT_ORDER, 실제 고정 좌석 순서)에 있는 테너/베이스는
    # 정원 여부와 상관없이 우선 배정한다.
    ...
    # 지정석에 없는 나머지 테너/베이스는 3열 정원을 먼저 다 채우고 4열로 넘기는 방식 대신,
    # 매 인원마다 3열/4열 중 더 적은 쪽에 채워서 처음부터 편차가 1 이하로 유지되게 한다.
    remaining_men = [("T", n) for n in people["T"]] + [("B", n) for n in people["B"]]
    people["T"], people["B"] = [], []
    for part, name in remaining_men:
