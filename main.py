...
def load_layout(req: LayoutModel):
    try:
        ss=get_sheets_service(); ds=get_drive_service(); fid=get_cached_file_id(ds)
        saved=load_layout_from_google_sheet(ss,fid,req.date_str,None)
        if not saved: return {"status":"not_found","date":req.date_str}
        seats=int(saved["total_seats"]); target=dt.date.fromisoformat(req.date_str)
        people=read_attendance(get_sheet_data(ss,fid,force_refresh=True),target)
        ap={p:len(people[p]) for p in ("S","A","T","B")}; rpc=[]
        for row in saved["rows"]:
            c={"S":0,"A":0,"T":0,"B":0,"ORG":0}
            for item in row:
                if item and item[0] in c:c[item[0]]+=1
            rpc.append(c)
        return {"status":"success","date":req.date_str,"total_seats":seats,"rows":saved["rows"],"saved_at":saved["saved_at"],"attending_count":sum(ap.values()),"attending_by_part":ap,"row_part_counts":rpc,"sungdoseok":{"S":[],"A":[],"T":[],"B":[]},"data_source":"Google Sheets API","layout_source":"Google Sheets saved layout","code_version":"2026-09-21-HCSS-v9"}
    except Exception as ex:
        logger.exception("수동배치 불러오기 실패"); raise HTTPException(status_code=500,detail=f"저장된 배치 불러오기 실패: {ex}")
