...
_diagnostic_target_date = None
_diagnostic_target_name = "이의헌"
_diagnostic_target_part = "베이스"

def download_excel_with_diagnostics(service, file_id, attempts=37, wait_seconds=5, previous_sha256=None):
    """Drive 본문 전파 지연을 고려해 XLSX 본문을 최대 약 3분간 재확인한다.

    Google Drive의 modifiedTime이 먼저 바뀌고 실제 파일 본문이 늦게 따라오는 경우를
    고려한다. revision alt=media는 사용하지 않고 검증된 get_media()만 사용한다.
    """
    import zipfile
    results = []
    last_bytes = None
    for i in range(1, attempts + 1):
        ...
        valid_xlsx = zipfile.is_zipfile(io.BytesIO(data))
        dbg = None
        if valid_xlsx and _diagnostic_target_date:
            dbg = debug_attendance_cell(data, dt.date.fromisoformat(_diagnostic_target_date), _diagnostic_target_name, _diagnostic_target_part)
        results.append({"attempt": i, "read_at": read_at, "size": len(data), "sha256": sha,
                        "valid_xlsx": valid_xlsx, "changed_from_previous": (previous_sha256 is None or sha != previous_sha256),
                        "attendance_debug": dbg})
        logger.info(f"[Drive 본문 재확인] #{i}/{attempts} size={len(data)} valid_xlsx={valid_xlsx} sha256={sha[:12]} changed={previous_sha256 is None or sha != previous_sha256}")
        if valid_xlsx:
            last_bytes = data
            # 이전 요청에서 읽었던 본문이 있고, 이번에는 새 본문이 도착했다면 즉시 종료한다.
            if previous_sha256 is None or sha != previous_sha256:
                return last_bytes, results
            # 이전 본문과 동일하면 Drive 전파 지연일 수 있으므로 계속 기다린다.
        else:
            logger.warning("[Drive 본문 오류] 내려받은 데이터가 XLSX(zip) 형식이 아닙니다. 잠시 후 재시도합니다.")
        if i < attempts:
            time.sleep(wait_seconds)
    # 3분 가까이 기다려도 새 본문이 없으면 마지막으로 받은 유효 XLSX를 사용한다.
    if last_bytes is not None:
        logger.warning("[Drive 본문 전파 지연] 새 본문이 확인되지 않아 마지막 유효 XLSX를 사용합니다.")
        return last_bytes, results
    raise ValueError("Google Drive에서 유효한 XLSX 본문을 받지 못했습니다. 잠시 후 다시 실행해 주세요.")


def get_excel_bytes(service, file_id, force_refresh=False):
    """Google Drive XLSX 본문을 가져옵니다.

    핵심 변경:
    - force_refresh=True에서는 modifiedTime을 새 파일 여부 판단 기준으로 사용하지 않습니다.
    - Drive의 modifiedTime 전파가 늦어도 XLSX 본문을 매번 직접 다시 받습니다.
    - 일반 실행(force_refresh=False)에서는 기존 캐시 최적화를 유지합니다.
    """
    meta = service.files().get(
        fileId=file_id, fields="id,modifiedTime,name,size"
    ).execute()
    mtime = meta.get("modifiedTime")

    if not force_refresh:
        with _cache_lock:
            cached_mtime = _cache["file_mtime"]
            cached_bytes = _cache["file_bytes"]
            if cached_bytes is not None and cached_mtime == mtime:
                logger.info(
                    f"[캐시] 파일 변경 없음 -> 캐시 재사용 "
                    f"(file_id={file_id}, mtime={mtime}, 파일명={meta.get('name')})"
                )
                return cached_bytes, mtime

    logger.info(
        f"[본문 다운로드] {'강제 새로고침' if force_refresh else '변경 감지'} "
        f"(file_id={file_id}, metadata_mtime={mtime})"
    )

    stream = download_excel_to_stream(service, file_id)
    file_bytes = stream.getvalue()

    # refresh 여부와 관계없이 다운로드한 실제 본문을 기준으로 저장
    with _cache_lock:
        _cache["file_bytes"] = file_bytes
        _cache["file_mtime"] = mtime

    logger.info(
        f"[다운로드 완료] {len(file_bytes)} bytes, "
        f"sha256={hashlib.sha256(file_bytes).hexdigest()}, mtime={mtime}"
    )
    return file_bytes, mtime

def debug_attendance_cell(file_bytes, target_date, target_name="이의헌", target_part="베이스"):
    """배치 결과와 별개로 실제 엑셀 셀 값을 검증한다."""
    ...
    for header_row in ws.iter_rows(min_row=1, max_row=2, values_only=True):
        for col_idx, cell_val in enumerate(header_row, start=1):
            if parse_cell_date(cell_val, target_date.year) == target_date:
                date_col = col_idx
                break
        if date_col: break
    if not date_col:
        return {"part": target_part, "name": target_name, "date": str(target_date), "date_col": None, "cell_value": None, "present": None}
    for row_idx, row_vals in enumerate(ws.iter_rows(min_row=3, values_only=True), start=3):
        name_val = row_vals[1] if len(row_vals) > 1 else None
        if str(name_val).strip() == target_name:
            val = row_vals[date_col - 1] if len(row_vals) >= date_col else None
            return {"part": target_part, "name": target_name, "date": str(target_date), "date_col": date_col, "cell_value": val, "present": val is True or (isinstance(val, str) and val.strip().upper() in ("TRUE","O","○","ㅇ","출석","참석","Y","YES","1","●")) or (isinstance(val,(int,float)) and val == 1), "row": row_idx}
    return {"part": target_part, "name": target_name, "date": str(target_date), "date_col": date_col, "cell_value": None, "present": None}

def get_cached_attendance(file_bytes, mtime, date_str, target_date, force_refresh=False):
    now = time.time()
    key = (mtime, date_str)
    if not force_refresh:
        with _cache_lock:
            ...
            if entry and (now - entry["cached_at"] < ATTENDANCE_CACHE_TTL):
                logger.info(
                    f"[출석 캐시 사용] key={key}, "
                    f"인원수={{p: len(v) for p, v in entry['data'].items()}}"
                )
                return entry["data"]
    else:
        logger.info(f"[강제 새로고침] 출석 파싱 캐시를 무시합니다. key={key}")
    logger.info(f"[출석 새로 파싱] key={key}")
    people = read_attendance(io.BytesIO(file_bytes), target_date)
    logger.info(f"[출석 파싱 결과] {date_str} -> { {p: len(v) for p, v in people.items()} }")
    with _cache_lock:
        # 오래된 캐시 정리 (메모리 누수 방지)
...
def read_attendance(file_stream, target_date):
    ...
    for sheet_name, part_code in sheet_map.items():
        ...
        for row_vals in ws.iter_rows(min_row=3, values_only=True):
            ...
            if c_val is True:
                is_present = True
            elif c_val is False or c_val is None:
                is_present = False
            elif isinstance(c_val, str):
                normalized = c_val.strip().upper()
                if normalized in ("TRUE", "O", "○", "ㅇ", "출석", "참석", "Y", "YES", "1", "●"):
                    is_present = True
                elif normalized in ("FALSE", "X", "결석", "불참", "N", "NO", "0", "×", "✕", "-"):
                    is_present = False
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-12-force-body-refresh-v5"}


@app.post("/api/allocate")
def run_allocation(req: RequestModel):
    global _diagnostic_target_date
    try:
        target = dt.date.fromisoformat(req.date_str)
        logger.info(f"[요청 시작] date={req.date_str}, total_seats={req.total_seats}, "
                    f"force_refresh={req.force_refresh}")

        previous_sha256 = None
        if req.force_refresh:
            with _cache_lock:
                old_bytes = _cache.get("file_bytes")
            if old_bytes:
                previous_sha256 = hashlib.sha256(old_bytes).hexdigest()
            logger.info(f"[force_refresh] 캐시 전체 초기화 (이전 SHA={previous_sha256[:12] if previous_sha256 else '없음'})")
            clear_all_cache()

        service = get_drive_service()
        try:
            configured_file_id = os.environ.get("DRIVE_FILE_ID", "").strip()

            if configured_file_id:
                # DRIVE_FILE_ID가 있으면 modifiedTime 정렬/파일명 검색을 완전히 건너뜁니다.
                file_id = configured_file_id
                logger.info(f"[고정 파일ID 사용] DRIVE_FILE_ID={file_id}")
            elif req.force_refresh:
                # 환경변수가 아직 없으면 기존 방식으로 한 번 파일을 찾습니다.
                # 단, 이후 본문 다운로드는 modifiedTime을 기준으로 캐시하지 않습니다.
                file_info = find_drive_file(service)
                file_id = file_info["id"]
            else:
                file_id = get_cached_file_id(service)

            _diagnostic_target_date = req.date_str
            if req.force_refresh:
                meta = service.files().get(fileId=file_id, fields="id,modifiedTime,name,size").execute()
                mtime = meta.get("modifiedTime")
                file_bytes, download_diagnostics = download_excel_with_diagnostics(service, file_id, attempts=37, wait_seconds=5, previous_sha256=previous_sha256)
                logger.info(f"[V3 진단] {download_diagnostics}")
            else:
                file_bytes, mtime = get_excel_bytes(service, file_id, force_refresh=False)
        ...
        people = get_cached_attendance(file_bytes, mtime, req.date_str, target, force_refresh=req.force_refresh)
        attendance_debug = debug_attendance_cell(file_bytes, target)
        ...
        return {
            "status": "success",
            "code_version": "2026-09-12-force-body-refresh-v5",
            "date": str(target),
            ...
            "attendance_debug": attendance_debug,
            "download_diagnostics": download_diagnostics,
            "server_read_at": dt.datetime.now(dt.timezone.utc).isoformat(),
