...
def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows):
    """기존 저장본을 삭제하지 않고 새 배치 저장본을 추가한다."""
    ...
    for r_idx, row in enumerate(rows, start=1):
        for slot_idx, item in enumerate(row, start=1):
            ...
            values.append([
                str(date_str), str(total_seats), now,
                str(r_idx), str(slot_idx), str(part), str(name)
            ])

    if not values:
        raise ValueError("저장할 좌석 배치 데이터가 없습니다.")

    # 중요: 기존 데이터를 clear()하지 않는다.
...
def verify_saved_layout(sheets_service, spreadsheet_id, date_str, total_seats, saved_at):
    """방금 저장한 데이터가 Google Sheets에 실제 존재하는지 재조회하여 검증한다."""
    try:
        ensure_layout_sheet(sheets_service, spreadsheet_id)
        ...
        logger.exception("저장 검증 조회 실패")
        return False

    target = (str(date_str).strip(), str(total_seats).strip(), str(saved_at).strip())
    return any(
        len(r) >= 7 and
        (str(r[0]).strip(), str(r[1]).strip(), str(r[2]).strip()) == target
        for r in values[1:]
    )

def load_layout_from_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats=None):
    try:
        ensure_layout_sheet(sheets_service, spreadsheet_id)
    ...
    found=[]
    for r in values[1:]:
        if len(r)<7 or str(r[0]).strip()!=str(date_str).strip(): continue
        try: found.append((str(r[2] or ""),int(r[1]),int(r[3]),int(r[4]),[r[5],r[6]]))
        except Exception: pass
    if not found: return None
    latest=max(x[0] for x in found); g=[x for x in found if x[0]==latest]
    rows=[[] for _ in range(max(x[2] for x in g))]
    for _,_,ri,si,item in sorted(g,key=lambda x:(x[2],x[3])): rows[ri-1].append(item)
    return {"rows":rows,"saved_at":latest,"total_seats":g[0][1]}
...
def save_layout(req: LayoutModel):
    try:
        ...
        saved_at=save_layout_to_google_sheet(
            sheets_service, file_id, req.date_str, req.total_seats, req.rows
        )
        if not verify_saved_layout(
            sheets_service, file_id, req.date_str, req.total_seats, saved_at
        ):
            raise HTTPException(
                status_code=502,
                detail="Google Sheets 저장 검증 실패: 기존 저장 데이터는 삭제하지 않았습니다."
            )
        return {
            "status":"success",
            "date":req.date_str,
            ...
            "saved_at":saved_at,
            "verified":True,
            "code_version":"2026-09-23-HCSS-storage-safe-v1"
        }
...
def load_layout(req: LayoutModel):
    try:
        ...
        saved=load_layout_from_google_sheet(sheets_service,file_id,req.date_str,None)
        if not saved:
        ...
        attending_by_part={"S":0,"A":0,"T":0,"B":0}
        attending_count=0
        try:
            target=dt.date.fromisoformat(req.date_str)
            values=get_sheet_data(sheets_service,file_id,force_refresh=True)
            people=read_attendance(values,target)
            attending_by_part={p:len(people[p]) for p in ("S","A","T","B")}
            attending_count=sum(attending_by_part.values())
        except Exception:
            logger.exception("출석정보 부가조회 실패 - 저장배치는 계속 반환")
        ...
        for row in saved["rows"]:
            ...
            row_part_counts.append(c)

        return {
            "status":"success","date":req.date_str,"total_seats":saved_seats,
            "rows":saved["rows"],"saved_at":saved.get("saved_at"),
            "attending_count":attending_count,
            "attending_by_part":attending_by_part,
            "row_part_counts":row_part_counts,
            "sungdoseok":saved.get("sungdoseok",{"S":[],"A":[],"T":[],"B":[]}),
            "data_source":"Google Sheets API",
            "layout_source":"Google Sheets saved layout",
            "code_version":"2026-09-21-HCSS-v11"
        }
    except HTTPException:
        raise
    except Exception as e:
        logger.exception("저장배치 불러오기 실패")
        raise HTTPException(status_code=500,detail=f"저장된 배치 불러오기 실패: {e}")
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-23-HCSS-storage-safe-v1"}
