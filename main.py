...
def get_latest_revision_info(service, file_id):
    """Drive에 등록된 XLSX의 최신 revision 정보를 가져온다.
    파일 modifiedTime 전파가 늦더라도 revision 단위로 최신 본문을 지정할 수 있게 한다.
    """
    revisions = []
    page_token = None
    while True:
        req = service.revisions().list(
            fileId=file_id,
            pageSize=100,
            fields="nextPageToken,revisions(id,modifiedTime,size,md5Checksum)"
        )
        if page_token:
            req = req.execute()
        else:
            req = req.execute()
        revisions.extend(req.get("revisions", []))
    ...
    def revision_time(r):
        try:
            return dt.datetime.fromisoformat(r.get("modifiedTime", "").replace("Z", "+00:00"))
        except Exception:
            return dt.datetime.min.replace(tzinfo=dt.timezone.utc)

    return max(revisions, key=revision_time)


def download_excel_revision_to_stream(service, file_id, revision_id):
    """특정 revision의 실제 XLSX 본문을 직접 내려받는다."""
    request = service.revisions().get(fileId=file_id, revisionId=revision_id, alt="media")
    file_stream = io.BytesIO()
    downloader = MediaIoBaseDownload(file_stream, request)
    done = False
    while not done:
...
def download_excel_with_diagnostics(service, file_id, attempts=3, wait_seconds=2):
    """강제 새로고침 시 Drive XLSX 본문을 revision 기준으로 반복 다운로드한다.
    일반 get_media()와 최신 revision 직접 다운로드를 모두 기록해 어느 단계에서
    오래된 본문이 나오는지 확인할 수 있게 한다.
    """
    results = []
    last_bytes = None
    ...
    try:
        latest_revision = get_latest_revision_info(service, file_id)
    except Exception as e:
        logger.warning(f"[V4 revision 조회 실패] {e}")
    ...
    for i in range(1, attempts + 1):
        ...
        if latest_revision and latest_revision.get("id"):
            try:
                stream = download_excel_revision_to_stream(
                    service, file_id, latest_revision["id"]
                )
                source = "revision"
            except Exception as e:
                logger.warning(f"[V4 revision 다운로드 실패] {e}; 일반 본문 다운로드로 전환")
        ...
        dbg = (
            debug_attendance_cell(
                data,
                dt.date.fromisoformat(_diagnostic_target_date),
                _diagnostic_target_name,
                _diagnostic_target_part
            )
            if _diagnostic_target_date else None
        )
    ...
    return last_bytes, results, latest_revision
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-12-force-body-refresh-v4"}
...
def run_allocation(req: RequestModel):
    ...
    try:
        ...
        if req.force_refresh:
            logger.info("[force_refresh] 캐시 전체 초기화")
            clear_all_cache()
        ...
        try:
            ...
            if req.force_refresh:
                ...
                mtime = meta.get("modifiedTime")
                file_bytes, download_diagnostics, latest_revision = download_excel_with_diagnostics(service, file_id, attempts=3, wait_seconds=2)
                logger.info(f"[V4 진단] revision={latest_revision}, diagnostics={download_diagnostics}")
            else:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-12-force-body-refresh-v4",
            "date": str(target),
