...
def parse_cell_date(val, target_year):
    ...
    # 1. datetime / date 객체인 경우
    if isinstance(val, dt.datetime):
        return val.date()
    if isinstance(val, dt.date):
        return val

    # 2. 엑셀 날짜 일련번호(int / float)인 경우
    ...
    # 3. 문자열 처리 ('8/30', '08/30', '8.30', '8월 30일' 등)
    val_str = str(val).strip()
    if not val_str:
        return None

    # YYYY-MM-DD, YYYY/MM/DD, YYYY.MM.DD 등 전체 연도 포함 형태
...
def read_attendance(file_stream, target_date):
    ...
    if not date_col:
        raise ValueError(f"해당 날짜({target_date.month}/{target_date.day})를 출석부 2행 또는 3행에서 찾을 수 없습니다.")

    people = {"S": [], "A": [], "T": [], "B": []}

    # 인원 데이터 읽기 (3행부터 시작하여 파트와 이름 탐색)
    for row in range(3, ws.max_row + 1):
        p_val = ws.cell(row=row, column=1).value
        n_val = ws.cell(row=row, column=2).value
        
        # 만약 1열/2열이 비어있다면 2열/3열 구조일 수 있으므로 유연하게 파싱
        part = str(p_val).strip().upper() if p_val else ""
        name = str(n_val).strip() if n_val else ""

        if part not in people and ws.cell(row=row, column=3).value:
            # 2열이 파트, 3열이 이름인 구도 대응
            part_alt = str(ws.cell(row=row, column=2).value or "").strip().upper()
            name_alt = str(ws.cell(row=row, column=3).value or "").strip()
            if part_alt in people:
                part, name = part_alt, name_alt
