...
def allocate(people_dict, total_seats, saved_rows=None, history_records=None):
    ...
    # priority_for(3/4, part)가 이미 해당 파트의 전체 출석자를 기본좌석 + 과거배치 점수 순으로 포함한다.
...
LAYOUT_RANGE = f"'{LAYOUT_SHEET_NAME}'!A1:J10000"
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-16-seat-summary-save-seed-history-v1"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-16-seat-summary-save-seed-history-v1",
            "date": str(target),
