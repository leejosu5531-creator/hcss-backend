...
def get_excel_bytes(service, file_id, force_refresh=False):
    """Google Drive의 현재 파일 내용을 가져옵니다.

    force_refresh=True이면 modifiedTime 캐시를 완전히 무시하고
    매번 파일 본문을 다시 다운로드합니다. 따라서 출석→결석,
    결석→출석 어느 방향의 변경도 즉시 반영할 수 있습니다.
    """
    ...
    if force_refresh:
        logger.info(
            f"[강제 새로고침] modifiedTime 캐시를 무시하고 "
            f"Google Drive 파일 본문을 다시 다운로드합니다. "
            f"(file_id={file_id}, mtime={mtime})"
        )
    else:
        logger.info(
            f"[다운로드] 파일 변경 감지 -> 새로 다운로드 "
            f"(file_id={file_id}, 이전 mtime={_cache.get('file_mtime')}, 새 mtime={mtime})"
        )
    ...
    logger.info(f"[다운로드 완료] {len(file_bytes)} bytes, mtime={mtime}")
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-11-force-drive-refresh-debug"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        try:
            file_info = find_drive_file(service) if req.force_refresh else None
            file_id = file_info["id"] if file_info else get_cached_file_id(service)
            file_bytes, mtime = get_excel_bytes(service, file_id, force_refresh=req.force_refresh)
        except Exception:
        ...
        return {
            "status": "success",
            "code_version": "2026-09-11-force-drive-refresh-debug",
            "date": str(target),
