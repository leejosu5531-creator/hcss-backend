...
def allocate(people_dict, total_seats, saved_rows=None):
    people = {p: list(people_dict[p]) for p in ("S", "A", "T", "B")}

    # 최근 수동배치의 좌석 순서를 자동배치의 "참고 좌석 세팅"으로 사용한다.
    # 출석하지 않은 사람은 무시하고, 현재 출석자의 기존 파트/이름을 우선한다.
    saved_rank = {}
    if saved_rows:
    ...
    def priority_for(row_num, part):
        ...
        return saved_first(base) + [n for n in saved_first(people.get(part, [])) if n not in base]
...
LAYOUT_RANGE = f"'{LAYOUT_SHEET_NAME}'!A1:J500"
...
def ensure_layout_sheet(sheets_service, spreadsheet_id):
    ...
    if LAYOUT_SHEET_NAME not in names:
        ...
        sheets_service.spreadsheets().values().update(
            spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:J1",
            valueInputOption="RAW",
            body={"values": [["date","total_seats","saved_at","row","slot","part","name","attending_count","attending_by_part","row_part_counts"]]}
        ).execute()


def save_layout_to_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats, rows,
                                attending_count=None, attending_by_part=None, row_part_counts=None):
    ...
    if current:
        ...
        for r in current[1:]:
            if len(r) >= 2 and r[0] == date_str and str(r[1]) == str(total_seats):
                continue
            kept.append(r)
    header = ["date","total_seats","saved_at","row","slot","part","name",
              "attending_count","attending_by_part","row_part_counts"]
    values = kept or [header]
    ...
    for r_idx, row in enumerate(rows, start=1):
        for slot_idx, item in enumerate(row, start=1):
            ...
            values.append([date_str, str(total_seats), now, str(r_idx), str(slot_idx), str(part), str(name),
                           str(attending_count if attending_count is not None else ""), p_json, rc_json])
    sheets_service.spreadsheets().values().clear(
        spreadsheetId=spreadsheet_id, range=LAYOUT_RANGE, body={}
    ).execute()
    sheets_service.spreadsheets().values().update(
        spreadsheetId=spreadsheet_id, range=f"'{LAYOUT_SHEET_NAME}'!A1:J{max(1,len(values))}",
...
def load_latest_layout_from_google_sheet(sheets_service, spreadsheet_id, total_seats=None):
    ...
    for r in values[1:]:
        ...
        candidates.append((ts, seats, r[0], ri, si, [r[5], r[6]], r[7] if len(r)>7 else "", r[8] if len(r)>8 else "{}", r[9] if len(r)>9 else "[]"))
...
def load_layout_from_google_sheet(sheets_service, spreadsheet_id, date_str, total_seats):
    ...
    attending_count = None; attending_by_part = None; row_part_counts = None
    for r in values[1:]:
        if len(r) >= 10 and r[0] == date_str and str(r[1]) == str(total_seats):
    ...
    return {"rows": rows, "saved_at": saved_at, "attending_count": attending_count,
            "attending_by_part": attending_by_part, "row_part_counts": row_part_counts}
...
def save_layout(req: LayoutModel):
    try:
        ...
        saved_at = save_layout_to_google_sheet(sheets_service, file_id, req.date_str, req.total_seats, req.rows,
                                               req.attending_count, req.attending_by_part, req.row_part_counts)
...
def load_layout(req: LayoutModel):
    try:
        ...
        return {"status":"success", "date":req.date_str, "total_seats":req.total_seats,
                ...
                "row_part_counts":saved.get("row_part_counts")}
    except Exception as e:
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-16-seat-summary-save-seed-crosspart-v1"}
    ...
    try:
        ...
        if req.use_saved_layout:
            try:
                ...
                logger.warning(f"최근 수동배치 참고 조회 실패(자동배치는 계속): {e}")

        rows, leftover, rt, org, row_part_counts, row_slots = allocate(
            people, req.total_seats, saved_rows=(saved_reference or {}).get("rows")
        )
        ...
        return {
            "status": "success",
            "code_version": "2026-09-16-seat-summary-save-seed-crosspart-v1",
            "date": str(target),
