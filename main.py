...
# ---------------------------------------------------------------------------
# 캐시 (구글 API 재호출 및 재조회로 인한 지연을 줄이기 위함)
# ---------------------------------------------------------------------------
...
_cache = {
    ...
    "content_mtime": None,      # 마지막으로 읽어온 시트 데이터의 수정시간
    "sheet_values": None,       # {시트이름: [[셀,...], ...], ...} 마지막으로 읽어온 원본 값
    "attendance_cache": {},     # (mtime, date_str) -> {"data": {...}, "cached_at": ts}
    "last_known_mtime": None,   # force_refresh로 캐시를 지워도 유지되는, 지금까지 본 최신 수정시간
                                 # (재시도 로직에서 "이번에도 그대로인지" 비교하는 기준)
}
...
def fetch_sheet_values(sheets_service, spreadsheet_id):
    ...
    ranges = [f"'{name}'!A1:Z500" for name in SHEET_NAMES]
...
def get_sheet_data(drive_service, sheets_service, file_id, expect_change_from=None,
                    max_retries=3, retry_delay=2.0):
    """수정시간(modifiedTime)만 가볍게 확인해서, 파일이 바뀌지 않았다면
    다시 읽어오지 않고 캐시된 시트 값을 재사용한다.

    expect_change_from이 주어지면(=force_refresh로 "방금 수정했다"는 신호가 있으면),
    구글 드라이브가 알려주는 수정시간이 그 값과 여전히 같을 때 곧바로 믿지 않고
    잠깐(retry_delay초) 기다렸다가 다시 확인한다."""
    mtime = None
    for attempt in range(max_retries + 1):
        meta = drive_service.files().get(fileId=file_id, fields="modifiedTime, name").execute()
        mtime = meta.get("modifiedTime")
        if expect_change_from is not None and mtime == expect_change_from and attempt < max_retries:
            logger.info(f"[재시도 대기] 수정시간이 새로고침 이전과 동일함(mtime={mtime}) "
                        f"-> 구글 드라이브 반영 지연으로 추정, {retry_delay}초 후 재확인 "
    ...
    with _cache_lock:
        _cache["last_known_mtime"] = mtime
        cached_mtime = _cache["content_mtime"]
        cached_values_exist = _cache["sheet_values"] is not None
        if cached_values_exist and cached_mtime == mtime:
            logger.info(f"[캐시] 변경 없음 감지 -> 캐시된 시트 값 재사용 "
                        f"(file_id={file_id}, mtime={mtime})")
            return _cache["sheet_values"], mtime
    logger.info(f"[시트 조회] 구글 시트 API로 새로 읽어옴 "
                f"(file_id={file_id}, 이전 캐시 mtime={cached_mtime}, 새 mtime={mtime})")
    sheet_values = fetch_sheet_values(sheets_service, file_id)
    with _cache_lock:
        _cache["sheet_values"] = sheet_values
        _cache["content_mtime"] = mtime
    total_rows = sum(len(v) for v in sheet_values.values())
    logger.info(f"[시트 조회 완료] 총 {total_rows}행, mtime={mtime}로 캐시 갱신")
    return sheet_values, mtime


def get_cached_attendance(sheet_values, mtime, date_str, target_date):
    now = time.time()
    key = (mtime, date_str)
    with _cache_lock:
        entry = _cache["attendance_cache"].get(key)
        if entry and (now - entry["cached_at"] < ATTENDANCE_CACHE_TTL):
            logger.info(f"[출석 캐시 사용] key={key}, 인원수={ {p: len(v) for p, v in entry['data'].items()} }")
            return entry["data"]
...
class CheckUpdatedRequest(BaseModel):
    known_mtime: Optional[str] = Field(default=None, description="이전에 확인된 파일 수정시간")


@app.post("/api/check-updated")
def check_updated(req: CheckUpdatedRequest):
    """구글 드라이브 파일의 수정시간만 가볍게 확인한다(다운로드/파싱 없음).
    구글 드라이브는 저장 직후 바로 반영되지 않고 수정시간 갱신까지 1~수 분 걸릴 수
    있어서, 프론트엔드가 이 엔드포인트를 짧은 간격으로 반복 호출해 변경 여부를
    가볍게 확인하는 용도다."""
    try:
        ...
        with _cache_lock:
            _cache["last_known_mtime"] = mtime
        changed = req.known_mtime is not None and mtime != req.known_mtime
        return {"mtime": mtime, "changed": changed}
    except Exception as e:
        logger.error(f"[check-updated 오류] {str(e)}")
...
def health_check():
    ...
    return {"status": "ok", "code_version": "2026-09-12-fix11-date-format-and-diagnostics"}
...
def run_allocation(req: RequestModel):
    try:
        ...
        expect_change_from = None
        if req.force_refresh:
            with _cache_lock:
                expect_change_from = _cache["last_known_mtime"]
            logger.info(f"[force_refresh] 캐시 전체 초기화 (직전까지 확인된 mtime={expect_change_from})")
            clear_all_cache()
        ...
        try:
            ...
            sheet_values, mtime = get_sheet_data(drive_service, sheets_service, file_id,
                                                  expect_change_from=expect_change_from)
        except Exception:
        ...
        people = get_cached_attendance(sheet_values, mtime, req.date_str, target)
        ...
        return {
            "status": "success",
            "code_version": "2026-09-12-fix11-date-format-and-diagnostics",
            "date": str(target),
