...
def allocate(people_dict, total_seats):
    ...
    row1_pool = [(part_of[n], n) for n in BASE_SEAT_ORDER[1] if n in names_in_pool]
    row2_pool = [(part_of[n], n) for n in BASE_SEAT_ORDER[2] if n in names_in_pool]
    listed = {n for _, n in row1_pool} | {n for _, n in row2_pool}
    unlisted_pool = [(p, n) for p, n in female_pool if n not in listed]

    r1, r2, female_overflow = _split_by_named_list(row1_pool, row2_pool, unlisted_pool, r, r)
    rows[0] = list(r1)
    rows[1] = list(r2)
