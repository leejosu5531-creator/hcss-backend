...
def allocate(people_dict, total_seats):
    ...
    # 좌우 반전 및 파트 내 정렬 순서 뒤집기 (B -> T -> A -> S -> ORG)
    ...
    for r_idx in range(4):
        ...
        # 2) 파트 표기는 파트가 시작하는 맨 앞 1회만 남김 (중복 제거)
        display_row = []
        last_part = None
        for p, name in sorted_row:
            if p != last_part:
                display_row.append((p, name))
                last_part = p
            else:
                display_row.append(("", name))
...
def run_allocation(req: RequestModel):
    try:
        ...
        return {
            "status": "success",
            "date": str(target),
            "attending_count": sum(attending_by_part.values()), # 총 출석 인원
            "attending_by_part": attending_by_part,            # 총 출석 파트별 인원
            "row_targets": rt,
            "organ_row": org + 1,
            "rows": rows,                                      # 파트표기 1회 적용 및 파트내 역순 정렬된 배치
            "row_part_counts": row_part_counts,                # 각 열별 파트 인원 합계
            "leftover": leftover
        }
